...
const quickLinks: FooterLink[] = [
  { key: 'home', page: 'home', hash: '#home' },
  { key: 'store', page: 'store' },
  { key: 'categories', page: 'home', hash: '#categories' },
...
export default function Footer({ onNavigate }: FooterProps) {
  ...
  const handleFooterLinkClick = (
    ...
    event.preventDefault()
    onNavigate(link.page, link.hash ? { hash: link.hash } : undefined)
  }

  const getFooterHref = (link: FooterLink) => {
    if (link.href) return link.href
    if (link.page === 'store') return '/store'
    if (link.page === 'about') return '/about'
    if (link.page === 'contact') return '/contact'
    if (link.page === 'cart') return '/cart'
    if (link.page === 'orders') return '/orders'
        ...
        <div className="container relative z-10 pt-28 pb-10 max-md:pt-24 max-md:pb-8">
          <div className="grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-4 lg:gap-10 max-md:gap-10">
            <div className="space-y-5" data-aos="fade-up">
              <a
                href="#home"
                onClick={(event) => handleFooterLinkClick(event, { key: 'home', page: 'home', hash: '#home' })}
                className="inline-flex items-center gap-2.5 font-heading text-[22px] font-extrabold text-white transition-colors duration-500 ease-out hover:text-fuchsia-300"
              >
              ...
              <div className="flex flex-wrap gap-2.5 pt-1">
                {socialLinks.map(({ key, href, Icon }) => (
                  <a
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-purple-100/80 transition-all duration-500 ease-out hover:border-fuchsia-400/30 hover:bg-fuchsia-500/10 hover:text-fuchsia-300 hover:shadow-[0_0_20px_rgba(217,70,239,0.25)]"
                  >
                    <Icon />
                  </a>
                ))}
